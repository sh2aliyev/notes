...
- Close:
  ...
  close(ch)
  ```
  > Closing a channel signals to the receiving goroutine that no more values will be sent to it. Close the channel after sending is complete to help avoid deadlocks.
...
The "Done Channel" pattern signals when a goroutine should stop. It uses a separate channel for control.
