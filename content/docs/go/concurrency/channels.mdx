...
- Close:
  ...
  <Callout type="warn">Closing a closed channel causes a panic.</Callout>
...
<Callout>Receiving from a closed unbuffered channel returns the zero value immediately.</Callout>
...
<Callout>A `select` statement is not a loop, it executes one case even if multiple are ready and then exits.</Callout>
...
<Callout>
  Always call the returned `cancel()` function when a context is no longer needed. Calling `cancel()` multiple times is
  safe.
</Callout>
