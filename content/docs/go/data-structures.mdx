...
- Create an instance:
  ...
  <Callout>Any field that is not explicitly specified is automatically assigned the zero value of its type.</Callout>
  ...
  > ```
  >
  > > When field names are omitted, values must be given in the exact declaration order, and every field must be provided.
...
- Embedded and nested structs:
  ...
  > - The nested struct remains a separate field and its fields must be accessed through that field name.
...
> By default, Go encodes fields using their names. In cases where fields must be capitalized but a different naming format is required in the output, struct tags allow you to define how fields appear in external formats, such as custom JSON names.
