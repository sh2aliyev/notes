...
- Declare:
  ...
  >
  > ```go
  > slice := make([]int, 5) // [0 0 0 0 0] (non-zero length but zero valued)
  > ```
...
- Initialize:
  ...
  You cannot simply declare a map with `var m map[string]int` and then assign values to it. If you try, you will get a `panic: assignment to entry in nil map`. To make a map ready to use, you must initialize it, either empty using the `make` function or with values.
