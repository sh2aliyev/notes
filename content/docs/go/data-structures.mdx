...
- Initialize:
  ...
  Declaring a map with `var m map[string]int` and then assigning values causes a `panic: assignment to entry in nil map`. Initialize the map first (empty or with values) before use.
