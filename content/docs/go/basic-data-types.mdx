...
- > Technically, a string is a read-only slice of bytes:
  ...
  > ```
  >
  > But some characters cannot be represented by a single byte:
  >
  > ```go
  > str := "é"
  ...
  >
  > Go strings are encoded using UTF-8. A string is a sequence of UTF-8 encoded bytes.
  >
  > UTF-8 uses a variable length encoding. A character may use 1 to 4 bytes depending on its Unicode code point.
  >
  ...
  > Extra:
  >
  > > When iterating with `range`, Go automatically decodes each UTF-8 character and returns its Unicode code point:
  > >
  > > ```go
