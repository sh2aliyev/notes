...
- Declaring multiple variables on a single line:
  ...
  If at least one identifier on the left-hand side is new, this syntax is allowed.
  ...
  The new identifiers are declared and the existing ones are reassigned.
...
<Callout type="warn">
  In Go, constants must be fully known at compile time. They cannot depend on variables, function calls, or any runtime
  values. Only expressions that the compiler can evaluate during compilation are allowed, and constants are limited to
  basic types like booleans, numbers, and strings.
</Callout>
