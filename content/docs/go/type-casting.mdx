...
Type casting in Go is the explicit conversion of a value from one compatible type to another.

```go
newValue := targetType(value)
```

> Go does not allow implicit type conversions. This prevents hidden bugs, keeps behavior predictable, and requires explicit control over how different types interact.
>
> ```go
> a := 5.6 // float64
> b := 10 // int
...
- Casting from `string` to `[]byte` or `[]rune` converts it into different representations:
