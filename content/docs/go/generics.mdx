---
title: Generics
---

Generics in Go allow you to write code that works with different data types while maintaining type safety.

## Generic Functions

Instead of creating separate functions for each type, you can define one function that operates on multiple types in a type-safe way.

> Syntax:
>
> ```go
> func functionName[T constraint](param T) returnType {
>     //
> }
> ```

Extra:

> Go does not support union types. You cannot write something like:
>
> ```go
> type number int | float64
> ```
>
> Instead, you can use an interface to define a type set for use as a generic constraint:
>
> ```go
> type number interface {
>     int | float64
> }
> ```
>
> > The `|` operator is only valid in type constraints. It specifies that a type parameter may be one of several types.
>
> then use it like:
>
> ```go
> func add[T number](a T, b T) T {
>     return a + b
> }
> ```
>
> > You can also use `|` directly without declaring a named interface:
> >
> > ```go
> > func add[T int | float64](a T, b T) T {
> >     return a + b
> > }
> > ```

> If you want the constraint to also accept user-defined types whose underlying type matches one of those types, use `~`:
>
> ```go
> type number interface {
>     ~int | ~float64
> }
> ```
>
> > Here, `~int` means `int` and any named type whose underlying type is `int`, and `~float64` means `float64` and any named type whose underlying type is `float64`.
>
> ```go
> type myInt int
>
> func add[T ~int | ~float64](a T, b T) T {
>     return a + b
> }
>
> func main() {
> 	var a myInt = 5
> 	var b myInt = 10
>
> 	fmt.Println(add(a, b)) // 15
> }
> ```

Examples:

1.  > ```go
    > func main() {
    >     fmt.Println(add(1, 2))     // 3
    >     fmt.Println(add(1.5, 2.3)) // 3.8
    > }
    >
    > func add[T int | float64](a T, b T) T {
    >     return a + b
    > }
    > ```
    >
    > > Without generics:
    > >
    > > ```go
    > > func main() {
    > >     fmt.Println(addInts(1, 2))       // 3
    > >     fmt.Println(addFloats(1.5, 2.3)) // 3.8
    > > }
    > >
    > > func addInts(a, b int) int {
    > >     return a + b
    > > }
    > >
    > > func addFloats(a, b float64) float64 {
    > >     return a + b
    > > }
    > > ```

2.  > ```go
    > func main() {
    >     fmt.Println(swap(2.5, 5))      // 5, 2.5
    >     fmt.Println(swap("Hello", 99)) // 99, Hello
    > }
    >
    > func swap[T any, U any](a T, b U) (U, T) {
    >     return b, a
    > }
    > ```

3.  > ```go
    > type gasEngine struct {
    >     hp int
    > }
    >
    > type electricEngine struct {
    >     kWh int
    > }
    >
    > type car[T gasEngine | electricEngine] struct {
    >     model  string
    >     year   int
    >     engine T
    > }
    >
    > func main() {
    >     gCar := car[gasEngine]{
    >         model: "Audi",
    >         year:  2018,
    >         engine: gasEngine{
    >             hp: 385,
    >         },
    >     }
    >
    >     eCar := car[electricEngine]{
    >         model: "Tesla",
    >         year:  2022,
    >         engine: electricEngine{
    >             kWh: 60,
    >         },
    >     }
    >
    >     fmt.Println(gCar, eCar) // {Audi 2018 {385}} {Tesla 2022 {60}}
    > }
    > ```

## Generic Types

Instead of creating separate versions of the same type for different data types, you can define a single type that can be reused with many types.

> Syntax:
>
> ```go
> type typeName[T constraint] struct {
>     fieldName T
> }
> ```

Example:

1.  > ```go
    > type List[T any] struct {
    > 	next *List[T]
    > 	val  T
    > }
    > ```
