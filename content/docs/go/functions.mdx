...
- `defer` depends only on the surrounding function scope:
  ...
  > `defer` is registered when execution reaches that line but runs when the surrounding function ends. It does not run when a block such as `if` or `for` ends, even if it appears inside them.
