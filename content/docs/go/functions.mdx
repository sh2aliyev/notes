...
- Pass-by Reference:
  ...
  Reference-Containing Types (`pass-by-reference`): Strings (immutable), Slices, Maps, Channels, Pointers, Functions,
  Interfaces.
...
1.  > Even when a value allows modification of shared underlying data, the variable itself is still passed by value. Reassigning the parameter inside a function only changes the local copy and does not affect the original variable in the caller.
    >
    >     ```go
