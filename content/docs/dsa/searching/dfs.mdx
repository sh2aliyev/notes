...
function adjMatrixDFS(vertex, graph, values, seen) {
  values.push(vertex);
  ...
  for (const v in connections) {
    if (connections[v] > 0 && !seen[v]) {
      adjMatrixDFS(v, graph, values, seen);
    }
  }
...
function adjListDFS(vertex, graph, values, seen) {
  values.push(vertex);
  ...
  for (let i = 0; i < connections.length; i++) {
    ...
    if (!seen[v]) {
      adjListDFS(v, graph, values, seen);
    }
  }
...
>
> > The red line shows the search flow starting from points `col: 0` and `row: 0`.
