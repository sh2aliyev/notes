...
A linked list is a data structure made of nodes where each node holds a value and a reference to the next node.
The nodes form a chain in memory which allows the list to grow or shrink easily by adjusting references.
...
class LinkedList {
  ...
  // (optional) Makes the list easier to view by converting it to an array:
  toArray() {
    const list = [];
